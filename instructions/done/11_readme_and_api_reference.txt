...
SCREENSHOTS ADDED -- 2026-08-12
    Taken after the visual work rather than before it, which is why they waited:
    depth cueing, the selection halo, the edge ink budget and the horizon grid
    all changed what the map looks like, so anything captured earlier would have
    been a picture of a version nobody can run.

    Five, all rendered headless from the committed cache: the map in dark and
    light with a gene selected, and the three levels of detail. 576 kB, and
    committed rather than generated by CI -- unlike the API page these need a GL
    context and the cache, so a runner cannot rebuild them.

    Two tests guard them: every image the README references must exist, because
    a broken image says the repository is unmaintained; and the directory must
    hold at least five, so deleting them fails rather than quietly shrinking the
    page.
