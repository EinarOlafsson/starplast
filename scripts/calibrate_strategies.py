...
GRID = {
    ...
    "network_training": {"layer": "GRAPH_TARGETS", "model": ["logistic", "embedding"],
                         "fraction": [0.25, 0.4]},
    "conformal_calls": {"target": "TARGETS", "alpha": [0.05, 0.1, 0.2],
                        "model": ["logistic", "kNN"]},
    "graph_convolution": {"target": "TARGETS", "hops": [1, 2, 3], "C": [0.1, 0.5]},
    "random_forest": {"target": "TARGETS", "trees": [100, 300], "min_leaf": [1, 2, 5]},
    "stacking": {"target": "TARGETS", "k": [10, 15, 30]},
    "conformal_values": {"target": "NUMBERS", "alpha": [0.1, 0.2], "model": ["boosted", "ridge"]},
}
...
SCORECARD_START = "<!-- scorecards:start -->"
SCORECARD_END = "<!-- scorecards:end -->"
...
def publish(out_dir: str, log=print, readme: str | None = None,
    ...
    if README_START in text:
        ...
        i = found.start()
        text = text[:i] + "\n".join(block) + "\n\n" + text[i:]
    cards = [SCORECARD_START, "",
             "### Scorecards: the same metrics for every strategy doing the same task", "",
             "Skill says how far above chance a strategy is; the scorecard says in what way. Each "
             "strategy performs one of six tasks, and every strategy performing a task reports "
             "that task's metrics, in this order, on the same hidden genes as its verdict: mean "
             "[95% CI] over held-out labels and seeds, at the default settings, on "
             "*T. gondii*. [Every metric explained, the techniques behind each strategy, "
             "*P. falciparum* and the tuned settings](docs/scorecards.md).", "",
             C.scorecard_markdown("Tg"), SCORECARD_END]
    if SCORECARD_START in text:
        a, rest = text.split(SCORECARD_START, 1)
        text = a + "\n".join(cards) + rest.split(SCORECARD_END, 1)[1]
    else:
        a, rest = text.split(README_END, 1)
        text = a + README_END + "\n\n" + "\n".join(cards) + rest
    ...
    log(f"wrote {doc}")
    cards_doc = os.path.join(os.path.dirname(doc), "scorecards.md")
    with open(cards_doc, "w") as fh:
        fh.write(scorecards_doc(meta))
    log(f"wrote {cards_doc}")
    return summary


def scorecards_doc(meta: dict) -> str:
    """The scorecard page: every metric and technique explained, every strategy's card."""
    from starplast import calibration as C
    from starplast import scorecard as SC
    from starplast import strategies as S
    from starplast import techniques as TQ
    one = lambda x: str(x).replace("|", "/").replace("\n", " ")
    out = ["# Strategy scorecards", "",
           f"Generated by `scripts/calibrate_strategies.py --publish` on {meta.get('date', '')} "
           f"from {meta.get('runs', 0):,} self-test runs.", "",
           "A self-test hides information that is already known, asks the strategy for it back, "
           "and judges the answer against the same procedure on shuffled data. The **verdict** "
           "rests on one metric. The **scorecard** reports, on the same hidden genes, every "
           "standard metric for the kind of task the strategy performs -- so strategies that do "
           "the same thing can be compared number by number, and a strategy's strengths and "
           "blind spots are visible: precise but narrow, good at the top of a list but not "
           "throughout, right about common classes and blind to rare ones.", "",
           "Every value below is a mean with a 95% interval: a two-stage bootstrap over held-out "
           "labels, then over runs (seeds) within a label, at the strategy's default settings "
           "unless the table says tuned. From Python: `strategies.metrics()`, "
           "`strategies.calibration(key)['default']['scorecard']`, and `TestResult.card()` for a "
           "test you run yourself.", "",
           "## The verdict block (every test)", "",
           "| Field | Meaning |", "|---|---|"]
    out += [f"| {label} | {one(text)} |" for _k, label, text in SC.VERDICT]
    out.append("")
    for task, spec in SC.TASKS.items():
        users = [s for s in S.catalog() if s.task == task]
        out += [f"## {task[0].upper() + task[1:]}", "",
                f"{spec.description} What is hidden: {spec.hidden}. Strategies: "
                + ", ".join(f"{s.number:02d}" for s in users) + ".", "",
                "| Metric | Definition | Range | Chance | How to read it |",
                "|---|---|---|---|---|"]
        for k in spec.metrics:
            m = SC.METRICS[k]
            out.append(f"| **{m.label}** | {one(m.definition)} | {one(m.range)} | "
                       f"{one(m.chance)} | {one(m.reading)} |")
        out.append("")
    for org, name in (("Tg", "Toxoplasma gondii"), ("Pf", "Plasmodium falciparum")):
        for setting, what in (("default", "at the default settings"),
                              ("tuned", "at the tuned setting (chosen on seeds 1-3, reported on "
                                        "seeds 4-5)")):
            out += [f"## *{name}*, {what}", "", C.scorecard_markdown(org, setting=setting), ""]
    out += ["## Techniques", "",
            "What each strategy's method is built from. The Strategies tab shows the same "
            "explanations beside each strategy; `strategies.techniques()` returns this table.", "",
            "| Technique | Kind | What it does | Why a strategy uses it | Used by |",
            "|---|---|---|---|---|"]
    for t in TQ.TECHNIQUES.values():
        used = ", ".join(f"{s.number:02d}" for s in S.catalog() if t.key in s.techniques)
        out.append(f"| **{t.name}** | {t.kind} | {one(t.what)} | {one(t.why)} | {used} |")
    return "\n".join(out) + "\n"
...
def calibration_doc(summary: dict, meta: dict) -> str:
    ...
    out = ["# Strategy calibration", "",
           ...
           "case is edge prediction: strategy 18 scores its held-out pairs against non-pairs "
           "drawn at random, and a random pair of genes is usually a pair of obscure genes, so "
           "much of what such a test measures is that well-connected genes are well connected. "
           "Its skill here is therefore an upper bound (strategies 16, 33 and 34 use "
           "degree-matched non-pairs instead). "
