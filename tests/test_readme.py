...
def test_the_readme_stays_short_enough_to_read_before_installing(readme):
    ...
    # So are the scorecard tables: one row per strategy per task.
    for block in ("calibration", "scorecards"):
        if f"<!-- {block}:start -->" in prose:
            head, rest = prose.split(f"<!-- {block}:start -->", 1)
            prose = head + rest.split(f"<!-- {block}:end -->", 1)[-1]
